...
Dynalinks.prototype.import_database =function(imported, key_compare)
{
    ...
    this.$emit("change", "import database", item);
...
{
    console.log("dynalinks emit changes");
    var arr = this.events[name];
    if (arr === undefined)  {
        this.events[name] = [];
        return;
...
Dynalinks.prototype.remove_features_by_link = function (category, id)
{
...
Dynalinks.prototype.update_record = function(old, new_value)
{
    
	//check tag changes and create new if need
